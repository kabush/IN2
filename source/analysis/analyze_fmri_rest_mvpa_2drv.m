...
logger([' Analyzing 2nd Deriv Predictions                 '],proj.path.logfile);
logger(['*************************************************'],proj.path.logfile);

%% ----------------------------------------
%% Set-up Directory Structure
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.analysis.analyze_fmri_rest_mvpa_2drv]);
    eval(['! rm -rf ',proj.path.analysis.analyze_fmri_rest_mvpa_2drv]);
    disp(['Creating ',proj.path.analysis.analyze_fmri_rest_mvpa_2drv]);
    eval(['! mkdir ',proj.path.analysis.analyze_fmri_rest_mvpa_2drv]);
end
...
for i = 1:numel(subjs)
    ...
    try
        ...
        load([proj.path.mvpa.fmri_rest_2drv_rgr,subj_study,'_',name,'_prds.mat']);
...
%% ----------------------------------------
%% overlay the individual 2nd deriv plot
...
%% Save out model
save([proj.path.analysis.analyze_fmri_rest_mvpa_2drv,'valence_2drv_mdl.mat'],'mdl');

%% ----------------------------------------
%% overlay the group 2 deriv plot
...
export_fig 'valence_mvpa_rest_2drv.png' -r300  
eval(['! mv ',proj.path.code,'valence_mvpa_rest_2drv.png ', ...
      proj.path.fig]);
...
%% Save out model
save([proj.path.analysis.analyze_fmri_rest_mvpa_2drv,'arousal_2drv_mdl.mat'],'mdl');
...
%% ----------------------------------------
%% explot hi-resolution figure
export_fig 'arousal_mvpa_rest_2drv.png' -r300  
eval(['! mv ',proj.path.code,'arousal_mvpa_rest_2drv.png ', ...
      proj.path.fig]);

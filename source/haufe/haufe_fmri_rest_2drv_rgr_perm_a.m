...
logger([' Global Perm. Testing of MVPA Hyperplanes 2drv a'],proj.path.logfile);
logger(['************************************************'],proj.path.logfile);

%% Set-up Directory Structure for fMRI betas
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.haufe.fmri_rest_2drv_rgr_a]);
    eval(['! rm -rf ',proj.path.haufe.fmri_rest_2drv_rgr_a]);
    disp(['Creating ',proj.path.haufe.fmri_rest_2drv_rgr_a]);
    eval(['! mkdir ',proj.path.haufe.fmri_rest_2drv_rgr_a]);
end
...
%         disp([subj_study,':',name,':i=',num2str(i)]);
        ...
        try
            ...
            d2h_indx = reshape(prds.a_indx.d2h',1,prod(size(prds.a_indx.d2h))); 
...
%             qlty = check_gm_img_qlty(d2h_img);
            ...
            if(qlty.ok)
                ...
                %% Fit classifier
                mdl = fitrsvm(d2h_img,d2h_trgs_a(label_ids),'KernelFunction',proj.param.mvpa.kernel);
                
                %% Construct Arousal Haufe tranform
                wts = mdl.Beta;
                haufe_a_wts = zscore(fast_haufe(d2h_img,wts,Nchunk));
    ...
    if(i>1)
        ...
        save([proj.path.haufe.fmri_rest_2drv_rgr_a,'grp_haufe_a_n=',num2str(i-1),'_of_N=',num2str(Nperm),'.mat'],'grp_haufe_a');

        if(i>2)
            eval(['! rm ',proj.path.haufe.fmri_rest_2drv_rgr_a,'grp_haufe_a_n=',num2str(i-2),'_of_N=',num2str(Nperm),'.mat']);
        end
        ...
        if(numel(row_ids_a)>0)
            ...
            save_nii(mu_a_haufe_nii,[proj.path.haufe.fmri_rest_2drv_rgr_a,'mu_haufe_a_N=',num2str(Nperm),'.nii']);
        ...
        if(numel(sig_ids_05_a)>0)
            ...
            save_nii(mu_perm_a_haufe_nii,[proj.path.haufe.fmri_rest_2drv_rgr_a,'mu_perm_haufe_a_N=',num2str(Nperm),'_05.nii']);
        end
        ...
        if(numel(sig_ids_01_a)>0)
            ...
            save_nii(mu_perm_a_haufe_nii,[proj.path.haufe.fmri_rest_2drv_rgr_a,'mu_perm_haufe_a_N=',num2str(Nperm),'_01.nii']);
        end
        ...
        if(numel(sig_ids_001_a)>0)
             ...
             save_nii(mu_perm_a_haufe_nii,[proj.path.haufe.fmri_rest_2drv_rgr_a,'mu_perm_haufe_a_N=',num2str(Nperm),'_001.nii']);
        end
        ...
        save([proj.path.haufe.fmri_rest_2drv_rgr_a,'grp_haufe_distr.mat'],'grp_haufe_a_dist');
    end

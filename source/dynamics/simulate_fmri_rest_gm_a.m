...
logger([' Simulate REST affect dynamics AROUSAL'],proj.path.logfile);
logger(['*************************************************'],proj.path.logfile);

%% ----------------------------------------
%% Set-up Directory Structure for fMRI betas
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.dyn.sim_rest_a]);
    eval(['! rm -rf ',proj.path.dyn.sim_rest_a]);
    disp(['Creating ',proj.path.dyn.sim_rest_a]);
    eval(['! mkdir ',proj.path.dyn.sim_rest_a]);
end
...
for loop = 1:loops
    ...
    for i = 1:numel(subjs)
        ...
        try
            ...
            h_indx = 6:215;
            h_trg = prds.a_hd(h_indx);

            clear prds;

            %% Load 2drv (AROUSAL) predictions
            ...
            %% Load 2drv NaN mask
            load([proj.path.mvpa.fmri_rest_2drv_rgr,subj_study,'_',name,'_a_nan_mask.mat'],'a_nan_mask');
            a_nan_mask_2drv = a_nan_mask;

            %% Add back NaN to equalize vector lengths
            for ii=1:length(a_nan_mask_2drv)
                if (a_nan_mask_2drv(ii) && ii==1)
                    d2h_out = [NaN; d2h_out(ii:end)];
                    d2h_trg = [NaN; d2h_trg(ii:end)];
                elseif (a_nan_mask_2drv(ii) && ii==210)
                    d2h_out = [d2h_out; NaN];
                    d2h_trg = [d2h_trg; NaN];
                elseif a_nan_mask_2drv(ii)
            ...
            %% Load 1drv NaN mask
            load([proj.path.mvpa.fmri_rest_1drv_rgr,subj_study,'_',name,'_a_nan_mask.mat'],'a_nan_mask');
            a_nan_mask_1drv = a_nan_mask;

            %% Add back NaN to equalize vector lengths
            for ii=1:length(a_nan_mask_1drv)
                if (a_nan_mask_1drv(ii) && ii==1)
                    dh_out = [NaN; dh_out(ii:end)];
                    dh_trg = [NaN; dh_trg(ii:end)];
                elseif (a_nan_mask_1drv(ii) && ii==210)
                    dh_out = [dh_out; NaN];
                    dh_trg = [dh_trg; NaN];
                elseif a_nan_mask_1drv(ii)
...
export_fig 'arousal_simulation.png' -r300  
eval(['! mv ',proj.path.code,'arousal_simulation.png ', ...
      proj.path.fig]);